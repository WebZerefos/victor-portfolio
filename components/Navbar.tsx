...
function Navbar() {
	...
	const navLinkClass = "flex items-center gap-1 font-medium text-lightText hover:text-designColor cursor-pointer duration-300 nav-link"
	...
	return (
		<section className='w-full h-20 lg:h-[8vh] fixed top-0 z-50 bg-bodyColor px-4 drop-shadow-lg'>
			<div className='max-w-container h-full mx-auto py-1 font-titleFont flex items-center justify-between'>
				...
				>
					<Image src={logoImg} alt='logo' className='w-12 rounded-full' />
				</motion.div>
				...
				{isHome && (
					...
					>
						<ul className='flex gap-12 text-xs'>
							<Link href='#home' onClick={onClickScroll} className={navLinkClass}>
								<li>Home</li>
							</Link>
							<Link href='#about' onClick={onClickScroll} className={navLinkClass}>
								<li>Sobre</li>
							</Link>
							<Link href='#project' onClick={onClickScroll} className={navLinkClass}>
								<li>Projetos</li>
							</Link>
							<Link href='#contact' onClick={onClickScroll} className={navLinkClass}>
								<li>Contato</li>
							</Link>
						</ul>
						<a href='resume.pdf' target='_blank'>
							<button className='px-4 py-2 rounded-md text-xs border border-designColor hover:border-white'>
								Resume
							</button>
						</a>
					</motion.div>
							...
							<div className='flex flex-col items-center gap-12 mt-20'>
								...
								>
									<a href='https://github.com/WebZerefos'>
										<motion.span whileHover={{ scale: [null, 1.5, 1.4] }} transition={{ duration: 0.1 }}
											className='w-10 h-10 text-xl bg-slate-700 rounded-full inline-flex items-center justify-center hover:text-designColor cursor-pointer transition-all duration-300'>
											<RiGithubLine />
										</motion.span>
									</a>
